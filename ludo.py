...
def comp_AI():
    global pieces , dice , turn , a , b , c , d , move_difference , diff_point , safe , last , behind_check , behind_check_move
    ...
    for i in range(4):
        ahead_danger = 0
        behind_danger = 0
        if pieces[turn][i][2] == 'h' and dice == 6:

            points[i] = 90
        elif pieces[turn][i][2] != 'h' and pieces[turn][i][1] + dice <= 57:
            heading = pieces[turn][i][3]
            place = pieces[turn][i][1]
            xcor = pieces[turn][i][0].xcor()
            ycor = pieces[turn][i][0].ycor()

            xcorb = xcor
            ycorb = ycor

            if pieces[turn][i][1] < 52:
                for m in range(dice):

                    if place in diff_point:
                        heading = move_difference[turn][diff_point.index(place)]
                    for bs in range(4):
                        for bi in range(4):
                            if bs != turn:
                                if int(pieces[bs][bi][0].xcor()) == int(xcor) and int(pieces[bs][bi][0].ycor()) == int(ycor):
                                    ahead_danger += 1
                    ...
                    if heading == 'u':
                        ...
                        xcor += math.cos(math.radians(45))*c
                        ycor -= math.sin(math.radians(45))*c

                    elif heading == 'dl':
                        xcor -= math.cos(math.radians(45))*d
                        ycor -= math.sin(math.radians(45))*d
            ...
            if pieces[turn][i][1] < 52:
                for m in range(6):
                    ...
                    bheading = behind_check_move[behind_check.index([int(xcorb),int(ycorb)])]
                    ...
                    if bheading == '-d':
                        ...
                        xcorb -= math.cos(math.radians(45))*c
                        ycorb += math.sin(math.radians(45))*c
                    elif bheading == '-ur':
                        xcorb += math.cos(math.radians(45))*d
                        ycorb += math.sin(math.radians(45))*d
                    elif bheading == '-dl':
                        xcorb -= math.cos(math.radians(45))*c
                        ycorb -= math.sin(math.radians(45))*c
                    elif bheading == '-dr':
                    ...
                    for bs in range(4):
                        for bi in range(4):
                            if bs != turn:
                                if int(pieces[bs][bi][0].xcor()) == int(xcorb) and int(pieces[bs][bi][0].ycor()) == int(ycorb):
                                    behind_danger += 1
            if pieces[turn][i][1] >= 42:
                ahead_danger = ahead_danger*6
                behind_danger = behind_danger*3
            #elif pieces[turn][i][1] >= 30:
                #ahead_danger = ahead_danger*2
                #behind_danger = behind_danger*2
            difference = behind_danger - ahead_danger
            points[i] += difference

            if pieces[turn][i][1] + dice == 57:
                points[i] = 80
            elif ((pieces[turn][i][1] + dice) in safe or (pieces[turn][i][1] + dice) in last) and pieces[turn][i][2] == 'n':
                points[i] = 70
            elif ((pieces[turn][i][1] + dice) in safe or (pieces[turn][i][1] + dice) in last) and pieces[turn][i][2] in ['s','t','b','l']:
                points[i] = 60
            elif behind_danger > ahead_danger + 3 and pieces[turn][i][2] == 'n':
                points[i] = 40
            elif ((pieces[turn][i][1] + dice) not in safe and (pieces[turn][i][1] + dice) not in last) and pieces[turn][i][2] == 's':
                points[i] = 30 - ahead_danger

            elif pieces[turn][i][2] != 'l':

                points[i] = 35 - ahead_danger


        if ahead_danger > 0 and pieces[turn][i][2] not in ['h','e','l']:
            KO = 0
            for bs in range(4):
                for bi in range(4):
                    if bs != turn :
                        if int(pieces[bs][bi][0].xcor()) == int(xcor) and int(pieces[bs][bi][0].ycor()) == int(ycor):
                            KO += 1
            if KO == 1:
                points[i] = 91 - behind_danger


    count = points.count(max(points))
    if count > 1 and max(points) == 90 :

        count_list = []
        for m in range(count):
             count_list += [points.index(max(points))]
             points[count_list[m]] = 0
        max_point = random.choice(count_list)
    elif count > 1 :
        todo = True
        if max(points)%5 != 0:
            todo = False
        ...
        position = []
        for m in count_list:
            position += [pieces[turn][m][1]]

        if todo == True:
            max_point = count_list[position.index(max(position))]
        else:
            max_point = count_list[position.index(min(position))]
    else:
        max_point = points.index(max(points))

    if pieces[turn][max_point][0].isvisible():
        object_click(pieces[turn][max_point][0].xcor(),pieces[turn][max_point][0].ycor())
    else:
        object_click(pieces[turn][max_point][4].xcor(),pieces[turn][max_point][4].ycor())
...
def show_move(option):
    global pieces , turn , circle , dice
    ...
    if option == 1:
        ...
        for i in range(4):
            if pieces[turn][i][2] == 'h' and dice == 6:
                check.append(i)
            elif (pieces[turn][i][2] != 'e' and pieces[turn][i][2] != 'h') and ((pieces[turn][i][1] + dice) <= 57):
                check.append(i)
        for i in check:
            if pieces[turn][i][0].isvisible():
                circle.goto(pieces[turn][i][0].xcor(),pieces[turn][i][0].ycor() - 15)
                circle.pendown()
                circle.circle(16)
                circle.penup()
            else:
                length = pieces[turn][i][4].width()/2
                circle.goto(pieces[turn][i][4].xcor(),pieces[turn][i][4].ycor() - length)
                circle.pendown()
                circle.circle(length + 0.6)
                circle.penup()
    elif option == 2:
        circle.reset()
        circle.hideturtle()
...
def dice_animation(x,y):
    global adice
    adice.setpos(x,y)
    adice.showturtle()
    for i in range(9):
        no = random.randint(1,6)
        adice.shape('dice'+str(no)+'.gif')
        sleep(0.1)
    adice.hideturtle()
...
def dice_check(x,y):
    ...
    '''if x < 0 and y > 0 :
        tim = datetime.now()
        str = tim.strftime('%d-%m-%Y_%H-%M-%S')
        print(str)
        save = open('.\\Save\\'+str+'.txt','w')
        save.writelines('rrrr')
        save.close()'''
